...
APP_NAME = "오프라인 PDF 내용 검색기 + 요약검색 (LLM 없음)"
...
# -----------------------------
# Korean-friendly optional n-gram(2-gram)
# -----------------------------
...
# - Sentence splitting + TF-IDF-ish scoring + diversity
# -----------------------------
...
def extractive_summary(text: str, max_sentences: int = 5, max_chars: int = 800) -> str:
    ...
    for i, toks in enumerate(sent_tokens):
        ...
        for w, c in tf.items():
            # idf
            idf = math.log((N + 1) / (df.get(w, 1) + 1)) + 1.0
            score += (c ** 0.5) * idf
        # 길이 패널티(너무 긴 문장 과대평가 방지)
        score /= (1.0 + (len(sents[i]) / 180.0))
        scores.append((score, i))
    scores.sort(reverse=True)

    # 다양성: 선택된 문장들 간 유사 토큰이 너무 많으면 스킵
    chosen: List[int] = []
    chosen_tok_sets: List[Set[str]] = []
    ...
    for _, idx in scores:
        if len(chosen) >= max_sentences:
            break
        tset = set(sent_tokens[idx])
        if not tset:
            continue
        too_similar = any(jacc(tset, prev) > 0.6 for prev in chosen_tok_sets)
        if too_similar:
            continue
        chosen.append(idx)
        chosen_tok_sets.append(tset)

    # 문서 흐름대로 정렬
    chosen.sort()
    out = " ".join(sents[i] for i in chosen)
    out = out.strip()
...
def build_doc_text_for_summary(pages_text: List[Tuple[int, str]], max_pages: int = 25) -> str:
    # 너무 긴 문서는 앞쪽 위주 + 조금 샘플링(요약 비용 제한)
    if not pages_text:
        return ""
    parts = []
            ...
            for idx, path in enumerate(pdfs, start=1):
                ...
                try:
                    ...
                    if self.build_summary:
                        ...
                        summ = extractive_summary(doc_text, max_sentences=self.summary_sentences, max_chars=900)
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Entry(top, textvariable=self.folder_var, width=66).pack(side="left", padx=6)
        ...
        self.summary_sent_var = tk.IntVar(value=5)
        ...
        ttk.Checkbutton(opt, text="문서 요약 생성/갱신(LLM 없이 추출요약)", variable=self.build_summary_var).grid(row=2, column=0, sticky="w", pady=(6, 0))
        ttk.Label(opt, text="요약 문장 수:").grid(row=2, column=1, sticky="e", padx=(12, 2), pady=(6, 0))
        ttk.Spinbox(opt, from_=2, to=12, textvariable=self.summary_sent_var, width=6).grid(row=2, column=2, sticky="w", pady=(6, 0))
        ...
        ttk.Entry(search, textvariable=self.query_var, width=76).grid(row=0, column=1, sticky="w", padx=6)
        ...
        self.tree.heading("snippet", text="스니펫")
        self.tree.heading("src", text="출처")
        self.tree.column("path", width=560, anchor="w")
        self.tree.column("page", width=70, anchor="center")
        self.tree.column("snippet", width=460, anchor="w")
        self.tree.column("src", width=70, anchor="center")
    ...
    def on_viewer_settings(self):
        ...
        ttk.Label(frm, text="(빈값: Sumatra 자동 → 없으면 기본 열기)").grid(row=0, column=2, sticky="w")
    ...
    def on_start_index(self):
        ...
        self.indexer = IndexerThread(
            ...
            summary_sentences=int(self.summary_sent_var.get()),
        )
        self.indexer.start()
        ...
        try:
            while True:
                ...
                if typ == "scan":
                    ...
                    self.status_var.set(f"[{done}/{total_changed}] 추출완료→DB: {path}")
                elif typ == "deleted":
                    ...
                    errs = payload
                    if errs:
                        self.status_var.set(f"인덱싱 완료(오류 {len(errs)}건)")
                    else:
