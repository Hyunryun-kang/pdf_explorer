...
APP_NAME = "오프라인 PDF 내용 검색기 (업그레이드)"
...
def char_2grams(text: str) -> str:
    ...
    # 공백도 포함하면 노이즈라 제거(단어 경계는 약해지지만 n-gram 목적상 OK)
    t2 = t.replace(" ", "")
    if len(t2) < 2:
        return t2
    ...
    for i in range(len(t2) - 1):
        grams.append(t2[i : i + 2])
    # 중복이 많아도 FTS에서 어느 정도 처리되지만,
    # 완전 중복 제거는 비용이 커서 간단히만:
    # 너무 길면 샘플링
...
class IndexDB:
    ...
    def replace_pages(self, doc_id: int, pages_text: List[Tuple[int, str]], build_ngram: bool):
        ...
        if build_ngram:
            # 새로 생성된 page_id들을 가져와서 grams 저장
            # (doc_id + page_no 기반으로 page_id 재조회)
            cur.execute("SELECT page_id, page_no, text FROM pages WHERE doc_id = ? ORDER BY page_no", (doc_id,))
            rows = cur.fetchall()
            grams_rows = []
            for r in rows:
                g = char_2grams(r["text"])
                grams_rows.append((int(r["page_id"]), g))
...
class App(tk.Tk):
    ...
    def on_search(self):
        ...
        try:
            ...
            for (path, page_no), (snippet, _score) in items[:500]:
                self.tree.insert("", "end", values=(path, page_no, snippet))

            msg = f"검색 완료: {len(items)}건"
            if fts_q:
                msg += f" | textFTS: {fts_q}"
